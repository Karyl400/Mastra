...
async function runProductionTestsWithMocks() {
  ...
  try {
    ...
    if (result.status === 'success') {
      ...
      results.push({ test: 'Création employé complet', status: 'PASS', duration });
    } else {
      const message = result.status === 'failed' ? String(result.error) : `Status: ${result.status}`;
  ...
  try {
    ...
    if (docResult.status === 'success') {
      ...
      const message = docResult.status === 'failed' ? String(docResult.error) : `Status: ${docResult.status}`;
  ...
  results.forEach((r) => {
    logger.info(`${r.test}: ${r.status} ${r.duration ? `(${r.duration}ms)` : ''}`, r.error ? { error: r.error } : {});
  });
  
  logger.info(`TOTAL: ${passed} PASS, ${failed} FAIL`);
