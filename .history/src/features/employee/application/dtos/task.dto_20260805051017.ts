...
const TASK_STATUS_TRANSITIONS: Record<TaskStatus, TaskStatus[]> = {
  ...
  [TaskStatus.InProgress]: [TaskStatus.Completed, TaskStatus.Blocked, TaskStatus.Cancelled],
  [TaskStatus.Blocked]: [TaskStatus.InProgress, TaskStatus.Cancelled],
  [TaskStatus.InReview]: [TaskStatus.Completed, TaskStatus.InProgress, TaskStatus.Cancelled],
...
const titleSchema = z
  ...
  .trim()
  .min(
    TASK_CONSTRAINTS.TITLE.MIN_LENGTH,
    `Title must be at least ${TASK_CONSTRAINTS.TITLE.MIN_LENGTH} characters`,
  )
  .max(
    TASK_CONSTRAINTS.TITLE.MAX_LENGTH,
...
const baseTaskSchema = z.object({
  ...
  tags: z
    .array(z.string().max(TASK_CONSTRAINTS.TAGS.MAX_LENGTH))
 ...
 */
const temporalCoherenceRefinement = z
  .object({
...
export class TaskValidator {
   ...
   */
  static async validateUpdate(taskId: string, data: unknown): Promise<UpdateTaskInput> {
  ...
  private static async validateBusinessRules(task: CreateTaskInput): Promise<void> {
    ...
    if (config.requiresAssignee && !this.hasAssignee(task)) {
      throw new TaskValidationError(`Tasks of type ${task.type} require an assignee`);
    }
   ...
   */
  static validateStatusTransition(from: TaskStatus, to: TaskStatus): void {
   ...
   */
  private static validateTemporalCoherence(currentTask: TaskDto, update: UpdateTaskInput): void {
    if (update.completedAt) {
      ...
      if (completedDate < createdDate) {
        throw new TaskValidationError('Completion date cannot be before creation date');
      }
    }
  }
  ...
  private static hasAssignee(
    task: Pick<TaskDto, 'assigneeId'> & { assigneeIds?: string[] },
  ): boolean {
    return !!(task.assigneeId || (task.assigneeIds && task.assigneeIds.length > 0));
  }
...
export const taskLifecycleHooks = {
   ...
   * Avant mise à jour
   */
  beforeUpdate: async (taskId: string, update: UpdateTaskInput): Promise<UpdateTaskInput> => {
...
export const taskCalculations = {
  ...
  getCompletionPercentage(task: TaskDto): number {
    ...
    if (task.type === TaskType.Onboarding && 'checklist' in task) {
      const checklist =
        (task as TaskDto & { checklist?: Array<{ completed: boolean }> }).checklist ?? [];
